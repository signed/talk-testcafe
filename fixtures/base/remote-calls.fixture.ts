...
const userAgentStub = RequestMock().onRequestTo(httpBinUserAgent).respond({}, 400, {
  'Access-Control-Allow-Origin': '*',
})

const requestLogger = RequestLogger(
  { url: httpBinUserAgent, method: 'GET' },
  {
    logRequestBody: true,
...
fixture('mock remote calls').page('http://localhost:8080/')
...
test('record calls', async (tc) => {
  ...
  await tc
    .expect(loggedRequest.request.url)
    .eql('https://httpbin.org/user-agent')
    .expect(loggedRequest.request.body)
    .eql('')
})
